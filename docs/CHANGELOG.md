...
## 2026-07-27 — Ronda de QA: seguridad, deuda técnica y bug real en Contabilidad Interna

- fix (seguridad): RLS de `bruck_*` solo tenía SELECT para clientes; la escritura dependía únicamente de `ci-client-auth.ts` a nivel de aplicación. Se agregaron políticas de INSERT/UPDATE/DELETE (`client_id = auth.uid()`) como red de seguridad a nivel de base de datos — ver `bruck-migration-v26.sql` (pendiente de correr en Supabase SQL Editor).
- fix (seguridad): `audit_logs` no tenía `organization_id` propio — el scoping por organización se aproximaba buscando los admins de la org y filtrando `user_id IN (...)`, lo cual se rompía si un admin cambiaba de organización. Ahora `logAudit()` recibe `organizationId` y se persiste directo — ver `bruck-migration-v27.sql` (pendiente de correr, incluye backfill).
- fix (bug latente, no explotable hoy): `openModal('mov', item)` en `contabilidad-panel.tsx` perdía `factura`/`comentario` al precargar el formulario desde un movimiento existente. Verificado con Playwright (2026-07-27): **hoy ningún botón de la UI llama a `openModal('mov', item)` con un item existente** — el único call site es "+ Nuevo movimiento" (siempre vacío) y la edición de movimientos existentes pasa por el modal "Clasificar" (`clasificarForm`, que no toca `factura`/`fecha`/`descripción`/`monto`). El fix queda igual de correcto para cuando se cablee un botón "Editar movimiento" real, pero no corrige nada que estuviera pasando en producción hoy. Gap de producto real: **no hay forma de corregir fecha/descripción/monto/factura de un movimiento ya creado sin borrarlo y recrearlo**.
- fix (deuda técnica): los 89 errores de `tsc --noEmit` (67 documentados en `PENDIENTES.md` + nuevos) se debían en su mayoría a una regresión de inferencia de tipos en `@supabase/supabase-js` 2.56+ (colapsa las filas a `never` bajo `strictNullChecks`). Se fijó la versión en `2.55.0` (última sin el bug) y se corrigieron los ~20 errores reales restantes (`profile` sin chequear null en varias rutas admin, target ES desactualizado en `tsconfig.json`). `tsc --noEmit` corre limpio.
- feat: ESLint configurado por primera vez en el proyecto (`next/core-web-vitals`) + CI básico en GitHub Actions (`.github/workflows/ci.yml`): typecheck, lint y build en cada push/PR a `develop`. Requiere agregar `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY` y `SUPABASE_SERVICE_ROLE_KEY` como secrets del repo para que el paso de build pase.
- fix: ruta muerta `/forgot-password` en `middleware.ts` (el flujo real usa un modal en `/login` + `/reset-password`).
- feat: UI del cliente en Contabilidad Interna ahora oculta los botones de crear/editar/eliminar cuando su plantilla de permisos (ej. "Auditor") no lo permite, en vez de mostrar controles que el backend ya rechazaba con 401.
- Migraciones `bruck-migration-v26.sql` y `v27.sql` **ya corridas** en Supabase (verificado con `pg_policies` y backfill de `audit_logs`, ambos OK).

## 2026-07-27 (2) — Upgrade Next.js 14→16 + botón real "Editar movimiento"

- **Upgrade Next.js 14.2.29 → 16.2.12** (React 18→19, `@types/react`/`@types/react-dom`→19), resolviendo los ~28 CVEs de `npm audit`. El código ya usaba el patrón `await params`/`await cookies()` en 45 archivos, lo cual redujo mucho el riesgo de breaking changes reales.
  - `middleware.ts` → `proxy.ts` (Next 16 renombró la convención; misma función, ahora exporta `proxy` en vez de `middleware`).
  - `next.config.js`: removida la key `eslint` (Next 16 desacopló lint del build, ya no la soporta).
  - `next lint` fue eliminado del CLI de Next 16 → `package.json` `"lint"` ahora corre `eslint .` directo, con `eslint.config.mjs` (flat config, formato que exige ESLint 9+) reemplazando `.eslintrc.json`.
  - `eslint-config-next` 16 trae nuevas reglas de "React Compiler readiness" (`react-hooks/immutability`, `preserve-manual-memoization`, `purity`, `set-state-in-effect`, `static-components`) que encontraron 18 findings reales en 6 archivos. El proyecto no usa el Compiler (no hay impacto en runtime hoy) — se bajaron a `warn` en `eslint.config.mjs` para no bloquear CI con una ola de hallazgos nuevos del upgrade; quedan como deuda técnica visible en `npm run lint`, no silenciados.
  - `npm audit`: de 4 vulnerabilidades altas a solo `xlsx` (sin fix, exposición baja ya documentada) + transitivos dev-only de ESLint y una dependencia interna de Next (postcss/sharp) sin nada más nuevo disponible.
  - Verificado: `tsc --noEmit` limpio, `npm run lint` limpio (0 errores), `npm run build` limpio, y smoke test en navegador (Playwright) con los 3 roles de la organización sandbox "Estudio Test" — sin errores de consola, `/admin` y Contabilidad Interna renderizan igual que antes.
- **Botón real "Editar movimiento"** en Contabilidad Interna (`contabilidad-panel.tsx`), cerrando el gap de producto encontrado en la ronda anterior: hasta hoy no había forma de corregir fecha/descripción/monto/factura/comentario de un movimiento ya cargado sin borrarlo y recrearlo. Se agregó un modal chico nuevo (`modal.type === 'mov-edit'`), separado del modal de carga masiva (`'mov'`, que sigue siendo solo para crear), reutilizando `movForm` + la lógica de `saveModal()` que ya soportaba PUT vs POST correctamente. Botón ✎ agregado en las tablas de Movimientos y Bancos, gateado por `canWrite`.
  - Verificado end-to-end en navegador con datos sembrados vía `service_role` en la organización sandbox: editar un movimiento, guardar, reabrir y confirmar que `factura`/`comentario`/`descripción` persistieron el cambio correctamente contra la base real. Datos de prueba eliminados al terminar.
