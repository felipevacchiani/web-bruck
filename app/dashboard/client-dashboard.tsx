          ...
          <nav style={{ padding:'16px 10px', flex:1, overflowY:'auto' }}>
            ...
            {[{value:'todos' as const,label:'Todos',icon:'📋',count:files.length},...FILE_CATEGORIES.map(c=>({...c,count:countBy(c.value)}))].map(cat=>(
              <button key={cat.value} onClick={()=>setActiveCategory(cat.value as any)}
                ...
                {cat.count>0 && <span style={{ fontSize:11, padding:'1px 7px', borderRadius:5, background:activeCategory===cat.value?(cat.value==='todos'?'rgba(255,255,255,0.12)':'rgba(49,174,121,0.2)'):'rgba(255,255,255,0.05)', color:activeCategory===cat.value?(cat.value==='todos'?'#d4d4d8':'#31AE79'):'#52525b' }}>{cat.count}</span>}
              </button>
            ))}
          </nav>
