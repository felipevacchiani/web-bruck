...
export default function FileViewer({ file, signedUrl, versions = [] }: Props) {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#080808' }}>
      <header style={{ flexShrink: 0, borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(10,10,10,0.98)', backdropFilter: 'blur(12px)', padding: '0 18px', height: 50, display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
          <button onClick={() => router.push('/dashboard')}
            style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#71717a', fontSize: 12, padding: '5px 12px', borderRadius: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
            ...
            <span style={{ color: 'white', fontSize: 13, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{file.name}</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          ...
          <span style={{ color: 'white', fontWeight: 900, letterSpacing: '0.2em', fontSize: 11 }}>BRUCK</span>
        </div>
      </header>
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        {loading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, background: '#080808' }}>
            ...
            <span style={{ color: '#52525b', fontSize: 13 }}>Cargando documento...</span>
          </div>
        )}
        {error && !loading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
            ...
            <p style={{ color: '#71717a', fontSize: 13 }}>No se pudo cargar el documento.</p>
