...
const INP: React.CSSProperties = { background:'rgba(18,23,20,0.04)', border:'1px solid rgba(18,23,20,0.14)', outline:'none', width:'100%', color:'#121714', fontSize:13, borderRadius:9, padding:'9px 12px', boxSizing:'border-box' }
...
const BTN_P: React.CSSProperties = { background:'linear-gradient(135deg,#31AE79,#27a06d)', color:'#121714', fontWeight:600, fontSize:13, padding:'9px 18px', borderRadius:9, border:'none', cursor:'pointer' }
    ...
    <div style={{ minHeight:'100vh', background:'#F3EFE5' }}>
      <header style={{ position:'sticky', top:0, zIndex:10, borderBottom:'1px solid rgba(18,23,20,0.08)', background:'rgba(243,239,229,0.92)', backdropFilter:'blur(12px)' }}>
        <div style={{ maxWidth:1000, margin:'0 auto', padding:'0 24px', height:52, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <Link href="/admin" style={{ color:'#858C87', fontSize:13, textDecoration:'none' }}>← Admin</Link>
      ...
      <main style={{ maxWidth:1000, margin:'0 auto', padding:'28px 24px' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
          <div>
            <div style={{ color:'#121714', fontSize:18, fontWeight:700 }}>Organizaciones</div>
            <div style={{ color:'#4E5651', fontSize:13, marginTop:2 }}>Cada organización es un consultor/líder independiente con sus propias empresas y usuarios.</div>
          </div>
          <button onClick={()=>setShowNew(true)} style={BTN_P}>+ Nueva organización</button>
        </div>

        {loading ? (
          <div style={{ color:'#858C87', fontSize:13, padding:40, textAlign:'center' }}>Cargando…</div>
        ) : orgs.length===0 ? (
          <div style={{ border:'1px solid rgba(18,23,20,0.08)', borderRadius:14, padding:48, textAlign:'center' }}>
            <div style={{ color:'#858C87', fontSize:13 }}>No hay organizaciones todavía.</div>
          </div>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {orgs.map(o => (
              <div key={o.id} style={{ background:'#FFFFFF', border:'1px solid rgba(18,23,20,0.16)', borderRadius:14, padding:18, boxShadow:'0 1px 3px rgba(18,23,20,0.05)' }}>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
                  <div>
                    <span style={{ color:'#121714', fontSize:15, fontWeight:600 }}>{o.name}</span>
                    <span style={{ color:'#858C87', fontSize:12, marginLeft:8 }}>/{o.slug}</span>
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                    <span style={{ fontSize:11, fontWeight:500, padding:'3px 9px', borderRadius:20, background:o.active?'rgba(52,211,153,0.08)':'rgba(18,23,20,0.04)', border:o.active?'1px solid rgba(52,211,153,0.2)':'1px solid rgba(18,23,20,0.12)', color:o.active?'#34d399':'#4E5651' }}>
                      {o.active?'Activa':'Inactiva'}
                    </span>
                    <button onClick={()=>toggleActive(o)} disabled={togglingId===o.id} style={{ background:'none', border:'1px solid rgba(18,23,20,0.14)', color:'#4E5651', fontSize:11, padding:'4px 10px', borderRadius:7, cursor:'pointer', opacity:togglingId===o.id?0.5:1 }}>
                      {togglingId===o.id ? '…' : (o.active ? 'Suspender' : 'Reactivar')}
                    </button>
                  </div>
                </div>
                <div style={{ display:'flex', gap:20, fontSize:12, color:'#4E5651', marginBottom: o.consultores.length ? 10 : 0 }}>
                  <span>{o.empresas_count} empresa{o.empresas_count!==1?'s':''}</span>
                </div>
                {o.consultores.length > 0 && (
                  <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                    {o.consultores.map(c => (
                      <div key={c.id} style={{ color:'#4E5651', fontSize:12 }}>👤 {c.full_name || c.email} <span style={{ color:'#858C87' }}>({c.email})</span></div>
                    ))}
                  </div>
                )}
              </div>
      ...
      {showNew && (
        <div style={{ position:'fixed', inset:0, zIndex:50, background:'rgba(0,0,0,0.7)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
          <div style={{ background:'#FFFFFF', border:'1px solid rgba(18,23,20,0.14)', borderRadius:16, padding:24, maxWidth:440, width:'100%' }}>
            <div style={{ color:'#121714', fontSize:16, fontWeight:700, marginBottom:4 }}>Nueva organización</div>
            <div style={{ color:'#4E5651', fontSize:12, marginBottom:18 }}>Crea el consultor y su cuenta de acceso inicial.</div>
            ...
            <form onSubmit={submit}>
              ...
              <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
                <button type="button" onClick={()=>{setShowNew(false);setError('')}} style={{ background:'none', border:'1px solid rgba(18,23,20,0.14)', color:'#4E5651', fontSize:13, padding:'9px 16px', borderRadius:9, cursor:'pointer' }}>Cancelar</button>
