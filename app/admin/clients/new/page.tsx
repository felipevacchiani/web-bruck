...
export default function NewClientPage() {
  ...
  const inputStyle: React.CSSProperties = { background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', outline: 'none', transition: 'border-color 0.15s', width: '100%', color: 'white', fontSize: 14, borderRadius: 10, padding: '10px 14px', boxSizing: 'border-box' }
  const labelStyle: React.CSSProperties = { color: '#a1a1aa', fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', display: 'block', marginBottom: 6 }
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '36px 24px' }}>
        ...
        <div style={{ background: 'rgba(14,14,14,0.8)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, overflow: 'hidden' }}>
          ...
          <div style={{ padding: 28 }}>
            {error && (
              <div style={{ display: 'flex', gap: 8, background: 'rgba(239,68,68,0.09)', border: '1px solid rgba(239,68,68,0.22)', borderRadius: 12, padding: '10px 14px', marginBottom: 22, color: '#f87171', fontSize: 13 }}>
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                <div>
                  ...
                  <input value={form.full_name} onChange={e => setForm(f => ({ ...f, full_name: e.target.value }))} placeholder="Juan Pérez" style={inputStyle}
                    onFocus={e => e.currentTarget.style.borderColor = 'rgba(49,174,121,0.55)'}
                    onBlur={e => e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'} />
                </div>
                <div>
                  <label style={labelStyle}>Empresa</label>
                  <input value={form.company} onChange={e => setForm(f => ({ ...f, company: e.target.value }))} placeholder="Acme S.A." style={inputStyle}
                    onFocus={e => e.currentTarget.style.borderColor = 'rgba(49,174,121,0.55)'}
                    onBlur={e => e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'} />
                </div>
              </div>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} required placeholder="cliente@empresa.com" style={inputStyle}
                  onFocus={e => e.currentTarget.style.borderColor = 'rgba(49,174,121,0.55)'}
                  onBlur={e => e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'} />
              </div>

              <div style={{ marginBottom: 28 }}>
                <label style={labelStyle}>Contraseña inicial <span style={{ color: '#ef4444' }}>*</span></label>
                ...
                <div style={{ color: '#52525b', fontSize: 12, marginTop: 6 }}>El cliente puede cambiarla luego desde "Olvidé mi contraseña"</div>
              </div>

              <div style={{ display: 'flex', gap: 10 }}>
                <button type="submit" disabled={loading}
                  style={{ background: 'linear-gradient(135deg, #31AE79, #27a06d)', color: 'white', fontWeight: 600, fontSize: 13, padding: '10px 22px', borderRadius: 10, border: 'none', cursor: loading ? 'not-allowed' : 'pointer', boxShadow: '0 4px 16px rgba(49,174,121,0.25)', opacity: loading ? 0.7 : 1, display: 'flex', alignItems: 'center', gap: 8 }}>
                  {loading ? <><span style={{ width: 13, height: 13, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white', borderRadius: '50%', display: 'inline-block', animation: 'spin 0.7s linear infinite' }} />Creando...</> : 'Crear cliente'}
                </button>
                <Link href="/admin" style={{ color: '#71717a', fontSize: 13, padding: '10px 18px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.09)', textDecoration: 'none', display: 'flex', alignItems: 'center' }}>
                  Cancelar
                </Link>
              </div>
            </form>
