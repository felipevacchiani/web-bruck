...
export default function NewClientPage() {
  ...
  const inp: React.CSSProperties = { background:'rgba(18,23,20,0.04)', border:'1px solid rgba(18,23,20,0.14)', outline:'none', transition:'border-color 0.15s', width:'100%', color:'#121714', fontSize:14, borderRadius:10, padding:'11px 14px', boxSizing:'border-box' }
        ...
        <div style={{ maxWidth:720, margin:'0 auto' }}>
          <div className="hpad" style={{ height:52, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              ...
              <span style={{ color:'#4E5651', fontSize:13 }}>Nuevo cliente</span>
            </div>
      ...
      <main style={{ maxWidth:720, margin:'0 auto' }}>
        <div className="nc-pad">
          <div style={{ marginBottom:28 }}>
            <div style={{ color:'#121714', fontSize:20, fontWeight:600 }}>Nuevo cliente</div>
            <div style={{ color:'#4E5651', fontSize:13, marginTop:4 }}>Completá los datos para crear la cuenta.</div>
          </div>

          <div style={{ background:'rgba(14,14,14,0.8)', border:'1px solid rgba(18,23,20,0.12)', borderRadius:18, overflow:'hidden' }}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginBottom:28 }}>
                  ...
                  <div style={{ position:'relative' }}>
                    ...
                    <button type="button" onClick={()=>setShowPass(!showPass)}
                      style={{ position:'absolute', right:12, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', color:showPass?'#31AE79':'#858C87', display:'flex', alignItems:'center', padding:2 }}>
                      {showPass
                ...
                <div style={{ display:'flex', gap:10 }}>
                  <button type="submit" disabled={loading}
                    style={{ flex:1, background:'linear-gradient(135deg,#31AE79,#27a06d)', color:'#121714', fontWeight:600, fontSize:14, padding:'12px', borderRadius:10, border:'none', cursor:loading?'not-allowed':'pointer', boxShadow:'0 4px 16px rgba(49,174,121,0.25)', opacity:loading?0.7:1, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
                    {loading ? <><span style={{ width:14, height:14, border:'2px solid rgba(18,23,20,0.3)', borderTopColor:'#121714', borderRadius:'50%', display:'inline-block', animation:'spin 0.7s linear infinite' }} />Creando...</> : 'Crear cliente'}
                  </button>
                  <Link href="/admin" style={{ color:'#4E5651', fontSize:13, padding:'12px 18px', borderRadius:10, border:'1px solid rgba(18,23,20,0.15)', textDecoration:'none', display:'flex', alignItems:'center', whiteSpace:'nowrap' }}>
                    Cancelar
                  </Link>
                </div>
              </form>
