      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 10, borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(10,10,10,0.97)', backdropFilter: 'blur(12px)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ color: '#52525b', fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase' }}>Auditoría</span>
          </div>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div className="page-pad">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
            <div>
              <div style={{ color: 'white', fontSize: 17, fontWeight: 600 }}>Registro de actividad</div>
              <div style={{ color: '#71717a', fontSize: 13, marginTop: 2 }}>{count || 0} evento{(count||0)!==1?'s':''} registrado{(count||0)!==1?'s':''}</div>
            </div>
              ...
              {(['', 'file_upload', 'file_delete', 'file_status_change', 'client_create', 'client_delete'] as const).map(a => (
                <Link
                  ...
                  style={{
                    ...
                    borderRadius: 7,
                    textDecoration: 'none',
                    border: action === a ? '1px solid rgba(49,174,121,0.4)' : '1px solid rgba(255,255,255,0.08)',
                    background: action === a ? 'rgba(49,174,121,0.1)' : 'rgba(255,255,255,0.03)',
                    color: action === a ? '#31AE79' : '#71717a',
                  }}
                >
          ...
          {!logs?.length ? (
            <div style={{ border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: '56px 24px', textAlign: 'center' }}>
              ...
              <div style={{ color: '#a1a1aa', fontWeight: 500 }}>No hay eventos registrados</div>
              <div style={{ color: '#52525b', fontSize: 13, marginTop: 6 }}>La actividad aparecerá aquí a partir de ahora</div>
            </div>
          ) : (
            <>
              {/* Tabla desktop */}
              <div className="audit-table" style={{ border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {logs.map((log: any, i: number) => {
                      const c = ACTION_COLORS[log.action as AuditAction] ?? { color:'#71717a', bg:'transparent', border:'rgba(255,255,255,0.06)' }
                      const details = log.details || {}
                      return (
                        <tr key={log.id} style={{ borderBottom: i===logs.length-1?'none':'1px solid rgba(255,255,255,0.04)' }}>
                          <td style={{ padding:'12px 16px', color:'#52525b', fontSize:12, whiteSpace:'nowrap' }}>{formatDatetime(log.created_at)}</td>
                          ...
                          <td style={{ padding:'12px 16px', color:'#52525b', fontSize:12, maxWidth:280 }}>
                            {details.name && <span style={{ color:'#a1a1aa' }}>{details.name}</span>}
                            {details.email && !details.name && <span style={{ color:'#a1a1aa' }}>{details.email}</span>}
                            {details.doc_status && <span style={{ color:'#71717a' }}> → {details.doc_status}</span>}
                            {details.files_deleted != null && <span style={{ color:'#71717a' }}> ({details.files_deleted} archivos)</span>}
                          </td>
                          <td style={{ padding:'12px 16px', color:'#52525b', fontSize:12 }}>{log.user_email || '—'}</td>
                        </tr>
                      )
                    })}
                  </tbody>
              ...
              <div className="audit-cards">
                {logs.map((log: any) => {
                  ...
                  const details = log.details || {}
                  return (
                    <div key={log.id} style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:12, padding:'12px 14px' }}>
                      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:10, marginBottom:6 }}>
                        ...
                        <span style={{ color:'#52525b', fontSize:11, whiteSpace:'nowrap' }}>{formatDatetime(log.created_at)}</span>
                      </div>
                      {(details.name || details.email) && (
                        <div style={{ color:'#a1a1aa', fontSize:13 }}>{details.name || details.email}</div>
                      )}
                      <div style={{ color:'#52525b', fontSize:12, marginTop:4 }}>{log.user_email}</div>
                    </div>
                  )
                })}
              </div>
              ...
              {totalPages > 1 && (
                <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, marginTop:20 }}>
                  {page > 1 && (
                    <Link href={`/admin/auditoria?page=${page-1}${action?`&action=${action}`:''}`} style={{ color:'#71717a', fontSize:12, border:'1px solid rgba(255,255,255,0.08)', padding:'6px 14px', borderRadius:8, textDecoration:'none' }}>
                      ← Anterior
                    </Link>
                  )}
